...
sudo chflags -f -vv uchg /private/var/vm/sleepimage
...
# MacBook built-in multitouch trackpad
# Enable Tap to click
defaults write com.apple.AppleMultitouchTrackpad Clicking -bool true
# Medium strength click
...
# Follow continuously with the pointer while zoomed in
...
# Require password 5 seconds after screen saver begins
...
# Enable snap-to-grid for icons on the desktop, and arrange-by-name in other icon views
...
# Make an array to remove default apps from the Dock that I don't care about.
eject_from_dock=(Siri Launchpad Contacts Notes Reminders Maps Photos Messages FaceTime Pages Numbers Keynote iBooks App%20Store)
awk_matcher=$(IFS="|" ; echo "${eject_from_dock[*]}")
i="$(defaults read com.apple.dock persistent-apps | grep _CFURLString\" | awk '/'$awk_matcher'/ {print NR}')"
# Array $i now contains the integer indexes of the apps that matched.
# Step through these in reverse order (and subtract 1) since removing low indexes will shift highter indexes.
for  j in `echo "$i" | tr " " "\n" | sort -gr`; do
